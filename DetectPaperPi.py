...
def get_roi2(cnt):
    ...
    cx = int(M["m10"]/M["m00"])
    cy = int(M["m01"]/M["m00"])
    #the coordinate (cx, cy) is the center of the given rectangle

    chkfill = [0, 0, 0, 0]
    #the list chkfill will check if some points are overlapped

    for point in cnt: #determine each points' attribute (top-left, top-right, bottom-left, bottom-right)
        if point[0][0] < cx and point[0][1] < cy:
            tl = point[0]
            chkfill[0] += 1
        elif point[0][0] > cx and point[0][1] < cy:
            tr = point[0]
            chkfill[1] += 1
        elif point[0][0] < cx and point[0][1] > cy:
            bl = point[0]
...
def frame_filter(frame, BIBLUR_SIZE=11, THR_SIZE=51, GAUBLUR_SIZE=(17, 17)):
    ...
    frame_hist = cv2.equalizeHist(frame_gray)
        #equalize histogram
    frame_blur = cv2.bilateralFilter(frame_hist, BIBLUR_SIZE, 51, 51)
        #bilateral filtering
    ...
    frame_edge = ~cv2.Canny(frame_thr, 160, 230)
        #detect edge with canny edge detecter
...
def find_paper(RESOLUTION=(320, 480), FRAME_RATE=20, MIN_AREA=0.2, MAX_AREA=0.7, WAIT_TIME=4, END_TIME=30):
        ...
        for cnt in cnts_apx:
            ...
            if area_total * MIN_AREA < area_cnt < area_total * MAX_AREA and cv2.isContourConvex(cnt) and len(cnt) == 4:
                ...
                if start[1] == 0: #when a paper is detected, available detecting will start after a given time
                    start[1] = time.time()
                    print('Will wait for {} secs'.format(WAIT_TIME))
                elif time.time() - start[1] > WAIT_TIME:
                    src_points, x = get_roi2(cnt)
                    ...
                    if x:
                        ...
                        break
                    print('Detecting failed! retrying...')
...
def capture_paper(src_points, src_resolution, RESOLUTION=(1280, 1920)):
    ...
    cv2.imwrite('paper_roi.jpg', img_roi)
